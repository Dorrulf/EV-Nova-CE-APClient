...
#ifdef BUILDING_MYDLL_DLL
#define MYDLL_API __declspec(dllexport)
#else
#define MYDLL_API __declspec(dllimport)
#endif

MYDLL_API void AP_Init(const char*, const char*, const char*, const char*);
MYDLL_API void AP_Init(const char*);
MYDLL_API bool AP_IsInit();

MYDLL_API void AP_Start();

// AP_Shutdown resets the library state to before initialization, and doesn't just disconnect!
MYDLL_API void AP_Shutdown();

struct MYDLL_API AP_NetworkVersion {
    int major;
    int minor;
    int build;
};

struct MYDLL_API AP_NetworkItem {
    int64_t item;
    int64_t location;
    int player;
    int flags;
...
struct MYDLL_API AP_NetworkPlayer {
    int team;
    int slot;
    std::string name;
    std::string alias;
...
MYDLL_API void AP_SetClientVersion(AP_NetworkVersion*);

/* Configuration Functions */

MYDLL_API void AP_EnableQueueItemRecvMsgs(bool);

MYDLL_API void AP_SetDeathLinkSupported(bool);

/* Required Callback Functions */

//Parameter Function must reset local state
MYDLL_API void AP_SetItemClearCallback(std::function<void()> f_itemclr);
//Parameter Function must collect item id given with parameter. Secound parameter indicates whether or not to notify player
MYDLL_API void AP_SetItemRecvCallback(std::function<void(int64_t,bool)> f_itemrecv);
//Parameter Function must mark given location id as checked
MYDLL_API void AP_SetLocationCheckedCallback(std::function<void(int64_t)> f_locrecv);

/* Optional Callback Functions */

//Parameter Function will be called when Death Link is received. Alternative to Pending/Clear usage
MYDLL_API void AP_SetDeathLinkRecvCallback(std::function<void()> f_deathrecv);
//Overload with the deathlink source and cause
MYDLL_API void AP_SetDeathLinkRecvCallback(std::function<void(std::string, std::string)> f_deathrecv);

// Parameter Function receives Slotdata of respective type
MYDLL_API void AP_RegisterSlotDataIntCallback(std::string, std::function<void(int)> f_slotdata);
MYDLL_API void AP_RegisterSlotDataMapIntIntCallback(std::string, std::function<void(std::map<int,int>)> f_slotdata);
MYDLL_API void AP_RegisterSlotDataRawCallback(std::string, std::function<void(std::string)> f_slotdata);

// Send LocationScouts packet
MYDLL_API void AP_SendLocationScouts(std::set<int64_t> const& locations, int create_as_hint);
// Receive Function for LocationInfo
MYDLL_API void AP_SetLocationInfoCallback(std::function<void(std::vector<AP_NetworkItem>)> f_locinfrecv);

/* Game Management Functions */

// Sends LocationCheck for given index
MYDLL_API void AP_SendItem(int64_t location);
MYDLL_API void AP_SendItem(std::set<int64_t> const& locations);

// Called when Story completed, sends StatusUpdate
MYDLL_API void AP_StoryComplete();

/* Deathlink Functions */

MYDLL_API bool AP_DeathLinkPending();
MYDLL_API void AP_DeathLinkClear();
MYDLL_API void AP_DeathLinkSend();
...
struct MYDLL_API AP_Message {
    AP_MessageType type = AP_MessageType::Plaintext;
    std::string text;
};

struct MYDLL_API AP_ItemSendMessage : AP_Message {
    std::string item;
    std::string recvPlayer;
};

struct MYDLL_API AP_ItemRecvMessage : AP_Message {
    std::string item;
    std::string sendPlayer;
};

struct MYDLL_API AP_HintMessage : AP_Message {
    std::string item;
    std::string sendPlayer;
    std::string recvPlayer;
    std::string location;
...
struct MYDLL_API AP_CountdownMessage : AP_Message {
    int timer;
};

/* Message Management Functions */

MYDLL_API bool AP_IsMessagePending();
MYDLL_API void AP_ClearLatestMessage();
MYDLL_API AP_Message* AP_GetLatestMessage();

MYDLL_API void AP_Say(std::string);
...
MYDLL_API int AP_GetRoomInfo(AP_RoomInfo*);
MYDLL_API AP_ConnectionStatus AP_GetConnectionStatus();
MYDLL_API std::uint64_t AP_GetUUID();
MYDLL_API int AP_GetPlayerID();
...
struct MYDLL_API AP_GetServerDataRequest {
...
struct MYDLL_API AP_DataStorageOperation {
    std::string operation;
    void* value;
};

struct MYDLL_API AP_SetServerDataRequest {
    AP_RequestStatus status;
    std::string key;
...
struct MYDLL_API AP_SetReply {
    std::string key;
    void* original_value;
    void* value;
};

struct MYDLL_API AP_Bounce {
...
MYDLL_API void AP_SetServerData(AP_SetServerDataRequest* request);
MYDLL_API void AP_GetServerData(AP_GetServerDataRequest* request);
 ...
 */
MYDLL_API void AP_BulkSetServerData(AP_SetServerDataRequest* requests);
MYDLL_API void AP_BulkGetServerData(AP_GetServerDataRequest* requests);

// Commits bulk server data requests
MYDLL_API void AP_CommitServerData();
...
MYDLL_API std::string AP_GetPrivateServerDataPrefix();
...
MYDLL_API void AP_RegisterSetReplyCallback(std::function<void(AP_SetReply)> f_setreply);
...
MYDLL_API void AP_SetNotify(std::map<std::string,AP_DataType>, bool = false);
// Single Key version of above for convenience
// AP_SetNotify will call AP_CommitServerData() and thus complete all pending serverdata requests
MYDLL_API void AP_SetNotify(std::string, AP_DataType, bool = false);

// Send Bounce package
MYDLL_API void AP_SendBounce(AP_Bounce);

// Receive Bounced packages. Disables automatic DeathLink management
MYDLL_API void AP_RegisterBouncedCallback(std::function<void(AP_Bounce)> f_bounced);

/* Gifting API Types */

struct MYDLL_API AP_GiftBoxProperties {
    bool IsOpen = false;
    bool AcceptsAnyGift = false;
    std::vector<std::string> DesiredTraits;
};

struct MYDLL_API AP_GiftTrait {
    std::string Trait;
    double Quality = 1.;
    double Duration = 1.;
};

struct MYDLL_API AP_Gift {
    std::string ID;
    std::string ItemName;
    uint64_t Amount;
    uint64_t ItemValue;
...
MYDLL_API AP_RequestStatus AP_SetGiftBoxProperties(AP_GiftBoxProperties props);
...
MYDLL_API std::map<std::pair<int,std::string>,AP_GiftBoxProperties> AP_QueryGiftBoxes();

// Get currently available Gifts in own gift box
MYDLL_API std::vector<AP_Gift> AP_CheckGifts();
...
MYDLL_API AP_RequestStatus AP_SendGift(AP_Gift gift);

// Accept a gift from the Giftbox
MYDLL_API AP_RequestStatus AP_AcceptGift(std::string id);
MYDLL_API AP_RequestStatus AP_AcceptGift(std::set<std::string> ids);

// Reject a gift from the Giftbox, and refund it.
MYDLL_API AP_RequestStatus AP_RejectGift(std::string id);
MYDLL_API AP_RequestStatus AP_RejectGift(std::set<std::string> ids);
